        ...
        int grayWidthStep)
{
    ...
    if((xIndex<width) && (yIndex<height))
    {
        ...
        output[gray_tid] = static_cast<unsigned char>(blue);
        output[gray_tid + 1] = static_cast<unsigned char>(green);
        output[gray_tid + 2] = static_cast<unsigned char>(red);
    }
}
...
void convert_to_self(const cv::Mat &input, cv::Mat &output)
{
    ...
    bgr_to_gray_kernel<<<grid,block>>>(d_input, d_output,
